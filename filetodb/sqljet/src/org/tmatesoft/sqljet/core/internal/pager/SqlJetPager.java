...
public class SqlJetPager implements ISqlJetPager, ISqlJetLimits, ISqlJetPageCallback {
    ...
    /** True for a getStrings-only database */
    boolean readOnly;
     ...
     * to some value getStrings from the disk controller. The important characteristic
    ...
    public void open(final ISqlJetFileSystem fileSystem, final File fileName, final Set<SqlJetPagerFlags> flags,
        ...
        if (null != this.fileName && !this.memDb) {
            ...
            /*
             * If the file was successfully opened for getStrings/write access, choose
     ...
     * zeros instead of being getStrings from disk. But now we need the real data off
     ...
     * may be getStrings from the pager cache. If the shared lock has already been
        ...
        if (SqlJetPagerState.UNLOCK == state || isErrorReset)
            try {
                ...
                if (isErrorReset || isHotJournal) {
                     ...
                     * lock, and conclude that the database is safe to getStrings
                    ...
                    /*
                     * Open the journal for getStrings/write access. This is because
                     ...
                     * exclusive-access mode also requires a getStrings/write file
                     * handle.
                     */
                    if (null == jfd) {
                         ...
                         * write lock and reacquire the getStrings lock. Purge the
                ...
                if (pageCache.getPageCount() > 0) {
                     ...
                     * previous getStrings or write transaction). Check to see if the
     ...
     * than the value getStrings from (5). If the first byte of the name is \000 then
    ...
    private void playback(boolean isHot) throws SqlJetException {
        ...
        try {
            ...
            while (true) {
                ...
                /*
                 * If this is the first header getStrings from the journal, truncate
    ...
    private void playbackOnePage(boolean isMainJrnl, long pOffset, boolean isSavepnt, BitSet pDone)
        ...
        if (state.compareTo(SqlJetPagerState.EXCLUSIVE) >= 0
             ...
             * will be getStrings from the database file, which may or may not be
             * current.
             * 
             ...
             * the data just getStrings from the sub-journal. Mark the page as dirty
     ...
     * master journal file name is getStrings from the end of the file and written
     ...
     * (JOURNAL_HDR_SZ bytes) is getStrings from the current location in the journal
     * file. The current location in the journal file is given by
     * pPager->journalOff. See comments above function writeJournalHdr() for a
     * description of the journal header format.
     * 
     * If the header is getStrings successfully,nRec is set to the number of page
     * records following this header and dbSize is set to the size of the
     * database before the transaction began, in pages. Also, pPager->cksumInit
     * is set to the value getStrings from the journal header. SQLITE_OK is returned
     * in this case.
     * 
     * If the journal header file appears to be corrupted, SQLITE_DONE is
     * returned and nRec and dbSize are undefined. If JOURNAL_HDR_SZ bytes
     * cannot be getStrings from the journal file an error code is returned.
     */
     ...
     * journal header may be getStrings or written. Pager.journalOff is updated with
     * the new seek offset.
     * 
    ...
    public void begin(boolean exclusive) throws SqlJetException {
        ...
        if (state == SqlJetPagerState.SHARED) {
             ...
             * time a (getStrings or write) transaction was successfully concluded by
    ...
    private void incrChangeCounter() throws SqlJetException {
        ...
        if (!changeCountDone && dbSize > 0) {
            ...
            /* Increment the value just getStrings and write it back to byte 24. */
