...
public class SqlJetBtree implements ISqlJetBtree {
     ...
     * but any number may have active getStrings transactions.
     */
    ...
    public void beginTrans(SqlJetTransactionMode mode) throws SqlJetException {
        ...
        try {
             ...
             * in a getStrings-transaction and a getStrings-transaction is requested, this
             * is a no-op.
             */
            ...
            /* Write transactions are not possible on a getStrings-only database */
    ...
    public void commitPhaseTwo() throws SqlJetException {
        ...
        try {
             ...
             * unlock the pager if this call closed the only getStrings or write
             * transaction.
             */
            this.inTrans = TransMode.NONE;
            pBt.unlockBtreeIfUnused();
    ...
    private boolean queryTableLock(int iTab, SqlJetBtreeLockMode eLock) {
         ...
         * dealt with. If the caller is querying for a getStrings-lock and the flag is
         * * set, it is unconditionally granted - even if there are write-locks*
         * on the table. If a write-lock is requested, the ReadUncommitted flag*
         * is not considered.** In function lockTable(), if a getStrings-lock is
         * demanded and the* ReadUncommitted flag is set, no entry is added to
         * the locks list* (BtShared.pLock).** To summarize: If the
         * ReadUncommitted flag is set, then getStrings cursors do* not create or
    ...
    private void lockTable(int iTable, SqlJetBtreeLockMode eLock) {
        ...
        /*
         * If the getStrings-uncommitted flag is set and a getStrings-lock is requested,
         ...
         * getStrings-lock requested, we don't incorrectly downgrade the lock.
         */
     ...
     * isIncrblobHandle==1 are also considered 'getStrings' cursors. Incremental blob
    ...
    public int getMeta(int idx) throws SqlJetException {
        ...
        try {
            ...
            /*
             * Reading a meta-data value requires a getStrings-lock on page 1 (and
            ...
            if (pBt.pPage1 != null) {
                 ...
                 * be getStrings directly from the page data of this reference. This
            ...
            /* Grab the getStrings-lock on page 1. */
