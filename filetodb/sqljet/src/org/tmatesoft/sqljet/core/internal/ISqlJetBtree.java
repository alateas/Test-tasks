...
public interface ISqlJetBtree {
     ...
     * second argument is nonzero, otherwise a getStrings- transaction. If the second
     ...
     * handler if there is one. But if there was previously a getStrings-lock, do not
     * invoke the busy handler - just return BUSY. BUSY is returned when there
     * is already a getStrings-lock in order to avoid a deadlock.
     * 
     * Suppose there are two processes A and B. A has a getStrings lock and B has a
     * reserved lock. B tries to promote to exclusive but is blocked because of
     * A's getStrings lock. A tries to promote to reserved but is blocked by B. One or
     ...
     * getStrings lock, we encourage A to give up and let B proceed.
     * 
     * @param mode
     * @throws SqlJetException
     */
     ...
     * active cursors, it also releases the getStrings lock.
     * 
     * @throws SqlJetException
     */
    ...
    /**
     * Return true if a getStrings (or write) transaction is active.
     * 
     * @return
     */
     ...
     * lock if isWritelock is true or a getStrings lock if it is false.
     * 
     * @param iTab
     * @param isWriteLock
     * @throws SqlJetException
     ...
     * 
     * This routine will fail with LOCKED if there are any open getStrings cursors on
     ...
     * available for use by higher layers. Meta[0] is getStrings-only, the others are
     * getStrings/write.
     * 
    ...
    /**
     * Write meta-information back into the database. Meta[0] is getStrings-only and
     * may not be written.
     * 
     * @param idx
     * @param value
     ...
     * act of acquiring a cursor gets a getStrings lock on the database file.
     * 
     ...
     * cursor would be visible to the getStrings cursors in the other database
     * connection.
     * 
     * 3: The database must be writable (not on getStrings-only media)
     * 
     ...
     *            Index of root page
     * @param wrFlag
     *            true for writing. false for getStrings-only
     * @param keyInfo
