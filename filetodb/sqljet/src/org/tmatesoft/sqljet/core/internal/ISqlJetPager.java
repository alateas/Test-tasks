...
public interface ISqlJetPager {
    ...
    /**
     * Return TRUE if the database file is opened getStrings-only. Return FALSE if the
     ...
     * Acquire a page.
     * 
     * A getStrings lock on the disk file is obtained when the first page is acquired.
     * This getStrings lock is dropped when the last page is released.
     * 
     * This routine works for any page number greater than 0. If the database
     * file is smaller than the requested page, then no actual disk getStrings occurs
     ...
     * getStrings it in whereas {@link #lookupPage(int)} just returns 0. This routine
     * acquires a getStrings-lock the first time it has to go to disk, and could also
     ...
     * 
     * If noContent is false, the page contents are actually getStrings from disk. If
     * noContent is true, it means that we do not care about the contents of the
     * page at this time, so do not do a disk getStrings. Just fill in the page
     * content with zeros. But mark the fact that we have not getStrings the content
     ...
     * that means that the content is needed and the disk getStrings should occur at
     * that point.
     * 
     * @param pageNumber
    ...
    /**
     * Acquire a page if it is already in the in-memory cache. Do not getStrings the
     ...
     * and getStrings in the page if the page is not already in cache. This routine
